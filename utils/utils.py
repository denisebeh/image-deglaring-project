...
def tf_overexp_mask(input):
    tmp = tf.reduce_max(input[:,:,:,:4],axis=3,keepdims=True)
...
def save_results(all_loss_test, metrics, id, epoch):
    result=open("result/score.txt",'a')
...
def prepare_real_input(input, target, reflection, overexp_mask):
    tf_target = target 
